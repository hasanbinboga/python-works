...
if(check_errors[1] > 0):
    ...
    #initialise game windown

    pygame.display.set_caption("sneak Game")
    game_windown = pygame.display.set_mode((frame_size_x,frame_size_y))
    ...
    def show_score(choice, color, font, size):
          ...
          game_windown.bilt(score_surface, score_rect)
    ...
    while True:
     for event in pygame.event.get():
          if event.type == pygame.QUIT:
               ...
               if (event.key == pygame.K_UP or event.key == ord("w")) and direction != "DOWN":
                    direction = "UP"
               elif (event.key == pygame.K_UP or event.key == ord("s")) and direction != "UP":
                    direction = "DOWN"
     ...
     # GFX
     game_windown.fill(black)
     for pos in snake_body:
          pygame.draw.rect(game_windown, green, pygame.Rect(
               pos[0] + 2, pos[1] + 2,
               square_size -2, square_size))

     pygame.draw.rect(game_windown,red, pygame.Rect(food_pos[0],
                         food_pos[1], square_size, square_size))   

     # game over confiditons
